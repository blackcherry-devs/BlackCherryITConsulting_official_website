...
export default function AnimatedLink({ href, children, className, onClick, target, ...props }: AnimatedLinkProps) {
  ...
  return (
    <Link href={href} className={className} onClick={handleClick} target={target} {...props}>
      {children}
    </Link>
  );
}
